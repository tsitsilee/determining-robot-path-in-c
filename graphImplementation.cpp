...
To be able to use anything here I had to first read and understand the functions and how they were being used 
most of the functions I needed were already there

however I had to edited the first find_path to ensure that it will allow my path to be printed to the output file by 
doing a vertex conversion to its corresponding coordinates(i,j)

most of the functions that needed me to write are in the assignment5Implementation.cpp file
...
//int vert_to_coordinates(int start,int map_dimension_y,int x,int y){

//}

/* adapted from original to return true if it is possible to reach the end from the start */
//function to find the path from the start to the goal
bool find_path(int start, int end,int map[][Max_M],int map_dimension_y, int parents[]) 
{   
	bool is_path;
	int i,j;
	// int map[][MAX_M];
	//int map_dimension_y, x,y;
	...
	else if ((start == end)) 
	{
		if (debug) 
		{
			...
			// vertex conversion to its corresponding coordinates(i,j)
			i = (int)(start - 1) / map_dimension_y;   //i cordinate
			j = start - (map_dimension_y * i) - 1;   //j coordinate
			map[i][j] = 4;
	...
	{
		is_path = find_path(start,parents[end],map,map_dimension_y,parent);
		if (debug)
		{
			printf(" %d",end);
			// vertex conversion to its corresponding coordinates(i,j)
			i = (int)(end - 1) / map_dimension_y;  //calculating for i
			j = end - (map_dimension_y * i) - 1;   //calaculating for j
			map[i][j] = 4;
		}
...
//int
//because I edited the first find_path had to also edited the second one which call the first
...
bool find_path(graph *g, int start, int end,int map[][Max_M],int map_dimension_y) {
	...
	else 
	{
		initialize_search(g);
		bfs(g, start);
		is_path = find_path(start, end, map, map_dimension_y, parent); // now call the version of find_path that has the parent array as an argument
		if (debug) printf("\n");
	}
	return(is_path);
}
